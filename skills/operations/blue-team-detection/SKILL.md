...
- Dynamic replay runs ONLY against authorized staging, mirroring the constraint in skills/code/injection/SKILL.md. The goal is validating detections, not re-exploiting production.
...
- Grep for forbidden payloads in current logs: raw tokens, passwords, full request bodies, session cookies. Redaction gaps cross-reference `skills/code/secrets-data-exposure/SKILL.md`; fix by hashing/truncation, never by deleting the event.
...
Audit honesty note: full-query-text logging makes every query greppable but costs real volume (queries repeat millions of times daily), inflates storage spend, drags latency at high QPS, and vacuums user data INTO logs (PII tension — resolve via hashing/truncation per Verification & Validation, cross-reference `skills/code/secrets-data-exposure/SKILL.md`). Prefer: capture query text only on ERROR, or sample at a fixed small rate.
...
|---|---|---|
| INJ | skills/code/injection/SKILL.md | R1 |
| AUTHN | skills/code/authn-session/SKILL.md | R2, R3 |
| AUTHZ | skills/code/authz-access-control/SKILL.md | R4 |
| SSRF | skills/code/ssrf-url-security/SKILL.md | R5 |
| DESER | skills/code/deserialization/SKILL.md | R6 |
| TOK/API | skills/server/api-token-security/SKILL.md | R7 |
...
**R1 — SQLi error-burst detection.** Replay the boolean-differential pair from skills/code/injection/SKILL.md against staging `/search`:
...
**R5 — SSRF metadata touch.** Replay the fetch-parameter PoC from skills/code/ssrf-url-security/SKILL.md pointing at the metadata address:
...
Apply to emails/usernames when policy requires; NEVER log raw tokens, passwords, cookies, or full request bodies. Truncation beats omission; hashing beats truncation for fields you must still group by. Full redaction rules: `skills/code/secrets-data-exposure/SKILL.md`.
...
- **PII-in-logs tension**: detection wants actor identity; privacy wants less PII. Resolution is hashing/truncation, NOT omission — pseudonymize emails/usernames (joinable), truncate UAs and tokens, never emit raw secrets or bodies. Cross-reference the redaction rules in `skills/code/secrets-data-exposure/SKILL.md`; when that module's redaction changes, re-run this module's replay suite because hashed fields must remain groupable.
...
- Host-side telemetry (sshd/sudo/auditd/integrity/shipping survival): server skillset `skills/server/logging-monitoring/SKILL.md` — authoritative for everything below the application layer; this module deliberately does not duplicate it.
- Red-team counterparts feeding the coverage matrix rows: `skills/code/injection/SKILL.md` (INJ), `skills/code/web-client/SKILL.md` (WEB), `skills/code/authn-session/SKILL.md` (AUTHN), `skills/code/authz-access-control/SKILL.md` (AUTHZ), `skills/code/ssrf-url-security/SKILL.md` (SSRF), `skills/code/file-handling/SKILL.md` (FILE), `skills/code/deserialization/SKILL.md` (DESER), `skills/code/api-security/SKILL.md` (API), `skills/code/business-logic-races/SKILL.md` (LOGIC), `skills/code/denial-of-service/SKILL.md` (DOS), `skills/server/api-token-security/SKILL.md` (TOK), `skills/code/llm-ai/SKILL.md` (LLM).
- Redaction rules resolving the PII-in-logs tension: `skills/code/secrets-data-exposure/SKILL.md`.
