...
Out of scope (cross-references): database engine hardening itself → DB (skills/server/db-server-hardening/SKILL.md); secret contents and env-file permissions → HSECRET; log shipping and alert-pipeline plumbing → LOGMON (skills/server/logging-monitoring/SKILL.md); TLS termination → TLS; firewall reachability proofs → FW.
...
Reuse the LOGMON exit-code→notify pattern (skills/server/logging-monitoring/SKILL.md):
