...
door; a subdomain pointing at a deleted bucket is a door someone else can own, which is the whole game in `skills/code/dns-takeover/SKILL.md`.
...
"attacker got in" into "attacker got into a small room." The kit enforces it app-side in `skills/code/authz-access-control/SKILL.md`
and `skills/code/cloud-iam/SKILL.md`, host-side in `skills/server/service-sandboxing/SKILL.md` and `skills/server/api-token-security/SKILL.md`. Its
nemesis is convenience (wildcard policies, shared admin accounts, permissive defaults), which is why misconfiguration gets
its own hunt (`skills/code/configuration-hardening/SKILL.md`).
...
anyway (`skills/operations/blue-team-detection/SKILL.md`), and incident response assumes even watchtowers miss
(`skills/operations/incident-response/SKILL.md`, `skills/operations/dfir-triage/SKILL.md`). Audit → harden → detect → respond is defense in depth expressed as a workflow rather than a diagram.
...
handler around a security decision returns. The stance shows up in `skills/code/configuration-hardening/SKILL.md` (TLS verification,
debug modes, proxy defaults), `skills/server/firewall-edge/SKILL.md` (default-deny posture), and `skills/code/crypto/SKILL.md`'s insistence
...
(`skills/operations/blue-team-detection/SKILL.md`); DFIR provides the first-two-hours playbook for a suspected-compromised host
(`skills/operations/dfir-triage/SKILL.md`); incident response covers the lifecycle through the lessons-feed-back step
(`skills/operations/incident-response/SKILL.md`); backups exist because ransomware is a clock problem (`skills/server/backup-dr/SKILL.md`). A team with great
...
**In this kit:** `skills/code/injection/SKILL.md` (slug INJ) owns the family (SQL, NoSQL, OS command, template, expression, LDAP,
...
**In this kit:** `skills/code/web-client/SKILL.md` (WEB) hunts all three XSS flavors, CSRF, clickjacking, unsafe `postMessage`
handlers, DOM clobbering, and client-side storage misuse; header-level depth lives in `skills/code/configuration-hardening/SKILL.md`.
...
**In this kit:** `skills/code/authn-session/SKILL.md` (AUTHN) covers login, storage, reset, MFA, remember-me, session lifecycle,
JWT/API-key handling; federation gets its own deep module, `skills/code/oauth-sso/SKILL.md` (SSO): OAuth/OIDC/SAML flow misuse,
...
**In this kit:** `skills/code/authz-access-control/SKILL.md` (AUTHZ) hunts BFLA/BOLA, vertical and horizontal escalation, tenant-
...
**In this kit:** `skills/code/cloud-iam/SKILL.md` (IAM) recovers IAM posture from repo/IaC artifacts (wildcard policies, PassRole
chains, CI OIDC scoping, IMDS hardening); host-side enforcement lands in `skills/server/kubernetes-cluster/SKILL.md` and
`skills/server/api-token-security/SKILL.md`. Fix philosophy: roles over static keys, scopes over wildcards, and treat the metadata
...
**In this kit:** `skills/code/ssrf-url-security/SKILL.md` (SSRF) catalogs sinks across mainstream HTTP-client libraries, bypass
taxonomy included, plus metadata-endpoint exposure; egress control pairs with `skills/server/firewall-edge/SKILL.md`. Fix
...
**In this kit:** `skills/code/secrets-data-exposure/SKILL.md` (SECRETS) sweeps working tree, history, build output, and logs; host
counterpart in `skills/server/host-secrets/SKILL.md`; API PII over-exposure overlaps `skills/code/api-security/SKILL.md`. Fix philosophy:
...
**In this kit:** `skills/code/file-handling/SKILL.md` (FILE) covers traversal, LFI/RFI, upload/ download/delete flaws, Zip Slip,
...
**In this kit:** `skills/code/deserialization/SKILL.md` (DESER) covers native serializers, XXE, YAML (`yaml.load` without
...
**In this kit:** `skills/code/api-security/SKILL.md` (API) covers mass assignment, rate/pagination caps, GraphQL/gRPC hygiene, key
mishandling, WebSocket/SSE access control; counting races overlap `skills/code/business-logic-races/SKILL.md`. Fix philosophy: explicit
...
**In this kit:** `skills/code/configuration-hardening/SKILL.md` (CONFIG) sweeps app/framework/ container/IaC/proxy defaults; host
baselines in `skills/server/linux-baseline/SKILL.md`, edge rules in `skills/server/firewall-edge/SKILL.md`, TLS termination in
`skills/server/tls-proxy/SKILL.md`, tunnel specifics in `skills/server/cloudflared-tunnel/SKILL.md`. Fix philosophy: hardened-by-default
...
**In this kit:** `skills/code/http-protocol/SKILL.md` (PROTO) hunts desync, Host-header abuse, cache poisoning/deception, parameter
...
**In this kit:** `skills/code/dns-takeover/SKILL.md` (DNS) does claimed-hostname inventory, dangling-record reasoning, provider
...
**In this kit:** Two modules split the angles cleanly. `skills/code/supply-chain/SKILL.md` (SUPPLY) audits manifests, lockfiles,
vendored code, and CI definitions for accidental weakness. `skills/code/malicious-code/SKILL.md` (MALCODE) hunts deliberate constructs
...
**In this kit:** `skills/code/denial-of-service/SKILL.md` (DOS) combines static inspection of regex batteries, parsers, algorithms with
bounded live probes; pagination abuse overlaps `skills/code/api-security/SKILL.md`. Fix philosophy: every byte a client sends must meet
...
**In this kit:** `skills/code/business-logic-races/SKILL.md` (LOGIC) covers workflow bypass, price/ amount tampering, TOCTOU, time-
...
**In this kit:** `skills/code/crypto/SKILL.md` (CRYPTO) spans algorithm choice, mode/nonce misuse, key management, password hashing,
...
**In this kit:** `skills/code/memory-safety/SKILL.md` (MEM) inventories native code, ranks it by attacker reachability, audits
...
**In this kit:** `skills/code/email-sms/SKILL.md` (MAIL) audits SPF/DKIM/DMARC evidence, sending infrastructure, inbound-mail
processing, verification/magic-link bypasses, OTP brute-force controls; account-takeover chains complete through
`skills/code/authn-session/SKILL.md` and `skills/code/oauth-sso/SKILL.md`. Fix philosophy: authenticate outbound mail cryptographically, throttle
...
**In this kit:** `skills/code/llm-ai/SKILL.md` (LLM) audits integrations (direct/indirect prompt injection, output handling, tool
...
**In this kit:** `skills/code/gaming-security/SKILL.md` (GAME) covers simulation authority, movement/action validation, economy
...
**Step 1: discovery.** During a code audit, the agent dispatches `skills/code/injection/SKILL.md` (INJ). Following §Taint Tracing
...
`skills/code/injection/SKILL.md` over the touched files. Only after all four does Fix Status move to `Verified-Fixed`.

**Step 5: the loop.** The finding enters tracking owned by `skills/operations/vuln-mgmt-process/SKILL.md` (VULN): prioritized against SLAs,
...
It is owned by `SKILL-OPERATIONS.md`, which coordinates four modules: `skills/operations/blue-team-detection/SKILL.md` (DETECT)
turns every class the red-team modules find into log signals and alert thresholds; `skills/operations/dfir-triage/SKILL.md` (DFIR) is the
first-hours forensic playbook for a suspected-compromised host; `skills/operations/incident-response/SKILL.md` (IR) audits whether you *have*
response capability and runs the lifecycle when reality strikes; `skills/operations/vuln-mgmt-process/SKILL.md` (VULN) keeps every finding
...
- **Newcomer:** this guide cover to cover, then `README.md`, then skim one module end to end; `skills/code/injection/SKILL.md` is the
  canonical specimen of the 12-section contract. Run a "quick pass" audit on a repo you own and read every artifact it
  produces.
- **Backend developer:** `SKILL-CODE.md` Phases 1–3 for method, then your stack's modules (`skills/code/injection/SKILL.md`,
  `skills/code/authz-access-control/SKILL.md`, `skills/code/authn-session/SKILL.md`, `skills/code/api-security/SKILL.md`), focusing on each module's §Remediation and
...
- **Ops/infrastructure:** `SKILL-SERVER.md` first, then `tools/README.md` (the sweep contract), then the twelve
  `skills/server/*` modules paired with their sweeps. Finish with `skills/operations/blue-team-detection/SKILL.md`; logging is your
  superpower.
