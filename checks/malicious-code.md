...
- Dependency version/vulnerability hygiene, lockfile-drift mechanics, registry-confusion preconditions, CI workflow-injection mechanics → `checks/supply-chain.md` (SUPPLY). Where this module says "review the postinstall scripts of ALL deps," SUPPLY carries the inventory command.
...
- Leaked credentials as such (a harvested key's storage hygiene) → `checks/secrets-data-exposure.md`.
...
Cross-module companions: dependency inventory, lockfile-diff mechanics, workflow injection, provenance verification → `checks/supply-chain.md` (SUPPLY); TLS configuration quality → CRYPTO module; leaked-credential storage hygiene → `checks/secrets-data-exposure.md`.
